...
use crate::builtins::system_controls::POFF;
use crate::def_dll::DllType;
...
pub enum UErrorMessage {
    ...
    FormError(String),
    UObjectIsNotAnArray,
    PlusAssignToObjectTypeValueNotAllowed,
    UnavailableFunction,
    ForceBoolError,
    MonitorNotFound,
...
impl fmt::Display for UErrorMessage {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::PlusAssignToObjectTypeValueNotAllowed => write_locale!(f,
                "+=で値を追加できるのは配列フィールドのみです",
                "Only array fields can have values added with +=",
            ),
